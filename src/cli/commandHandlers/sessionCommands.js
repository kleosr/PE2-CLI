...
export async function handleImport(ctx) {
    ...
    try {
        const fileContents = fs.readFileSync(importPath, 'utf-8');
        console.log(themeManager.color('success')('✓ File imported successfully'));
        console.log(themeManager.color('info')(`📄 Content preview: ${formatContentPreview(fileContents, { maxLength: 300 })}`));
        return fileContents;
    } catch (error) {
...
function displayPromptPreviews(themeManager, prompts) {
...
export async function handleBatch(ctx) {
    ...
    try {
        const prompts = fs.readFileSync(batchPath, 'utf-8')
            .split('\n')
            .filter(p => p.trim());
        console.log(themeManager.color('info')(`Found ${prompts.length} prompts to process.`));

        if (prompts.length > 0) {
            displayPromptPreviews(themeManager, prompts);
        }
